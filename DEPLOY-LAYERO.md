...
Проект работает как Next.js SSR-приложение с API-маршрутами заказа и расчёта доставки. Расстояние рассчитывается на сервере по формуле Haversine с коэффициентом 1,3. При оформлении заказа сервер Layero параллельно отправляет заявку на электронную почту и в Telegram.
...
YANDEX_SUGGEST_API_KEY=ключ_геосаджеста
YANDEX_GEOCODER_API_KEY=ключ_геокодера
DELIVERY_QUOTE_SECRET=случайная_строка_не_короче_32_символов
