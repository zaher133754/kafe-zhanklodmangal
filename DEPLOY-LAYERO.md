...
`TELEGRAM_PROXY_URL` в Layero не задавайте: сервер хостинга должен обращаться к Telegram напрямую. Эта переменная нужна только для локального запуска из сети, где Telegram доступен через прокси.
